...
class AudioPlayer:
    ...
    def create_widgets(self):
        ...
        self.play_button = tk.Button(button_frame, text="▶ Play", command=self.play_pause, bg="lightgreen",
                                     font=("Arial", 12, "bold"))
        self.play_button.grid(row=0, column=0, padx=5)

        self.stop_button = tk.Button(button_frame, text="⏹ Stop", command=self.stop, bg="indianred1",
                                     font=("Arial", 12, "bold"))
        self.stop_button.grid(row=0, column=1, padx=5)

        self.prev_track_button = tk.Button(button_frame, text="⏮ Previous", command=self.play_prev_track, bg="lightblue",
                                           font=("Arial", 12, "bold"))
        self.prev_track_button.grid(row=0, column=2, padx=5)

        self.next_track_button = tk.Button(button_frame, text="⏭ Next", command=self.play_next_track, bg="lightblue",
                                           font=("Arial", 12, "bold"))
        ...
        self.add_button = tk.Button(playlist_button_frame, text="➕ Add to Playlist", command=self.add_to_playlist,
                                    bg="lightgrey", font=("Arial", 12))
        self.add_button.grid(row=0, column=0, padx=5)

        self.create_playlist_button = tk.Button(playlist_button_frame, text="📂 Create Playlist", command=self.create_playlist,
                                                bg="lightgrey", font=("Arial", 12))
        self.create_playlist_button.grid(row=0, column=1, padx=5)

        self.load_playlist_button = tk.Button(playlist_button_frame, text="📁 Load Playlist", command=self.load_playlist,
                                              bg="lightgrey", font=("Arial", 12))
        self.load_playlist_button.grid(row=0, column=2, padx=5)

        self.save_playlist_button = tk.Button(playlist_button_frame, text="💾 Save Playlist", command=self.save_playlist,
                                              bg="lightgrey", font=("Arial", 12))
        self.save_playlist_button.grid(row=0, column=3, padx=5)

        self.clear_playlist_button = tk.Button(playlist_button_frame, text="🗑 Clear Playlist", command=self.clear_playlist,
                                               bg="lightgrey", font=("Arial", 12))
        self.clear_playlist_button.grid(row=0, column=4, padx=5)

        self.remove_track_button = tk.Button(playlist_button_frame, text="❌ Remove Track",
                                             command=self.remove_track_from_playlist, bg="lightgrey", font=("Arial", 12))
        ...
        self.search_button = tk.Button(search_frame, text="🔍 Search", command=self.search_tracks, bg="lightgrey",
                                       font=("Arial", 12))
        self.search_button.grid(row=0, column=1, padx=5)

        self.track_info_button = tk.Button(self.root, text="ℹ Track Info", command=self.display_track_info, bg="lightgrey",
                                           font=("Arial", 12))
    ...
    def create_playlist(self):
        ...
        if name:
            if name in self.playlists:
                messagebox.showwarning("Warning", f"A playlist with the name '{name}' already exists.")
            else:
    ...
    def set_position(self, position):
        if self.current_track is not None:
            ...
            self.update_timeline_slider(new_pos)
    ...
    def track_time_update(self):
        ...
        while not self.stop_timeline_thread:
            if self.current_track is not None and not self.paused:
                ...
                self.root.after(0, self.update_timeline_slider, current_pos)

                track_length = pygame.mixer.Sound(self.current_playlist.tracks[self.current_track]).get_length()
                if current_pos >= track_length - 1:  # Adjust for any latency
                    if self.repeat_track:
                        self.play_track(self.current_track)
                    else:
                        next_track_index = self.current_track + 1
                        if next_track_index >= len(self.current_playlist.tracks):
...
#    def update_timeline_slider(self, current_pos):
#        if self.current_track is not None:
#            track = self.current_playlist.tracks[self.current_track]
#            length = pygame.mixer.Sound(track).get_length()
